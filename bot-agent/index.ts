// Telegram-бот кабинета партнёра (@preipoprobot).
// Переиспользует ту же БД (dev.db) и финансовую логику, что и веб-кабинет /agent.
// Запуск: npx tsx bot-agent/index.ts
import { readFileSync } from "node:fs";
import { scryptSync, timingSafeEqual } from "node:crypto";
import { Bot, InlineKeyboard } from "grammy";
import { PrismaClient } from "../src/generated/prisma/client";
import { PrismaBetterSqlite3 } from "@prisma/adapter-better-sqlite3";
import { portfolio, saleMetrics } from "../src/lib/agent-calc";
...
bot.on("message:text", async (ctx) => {
  ...
  if (s.step === "sale_pick" || s.step === "sale_company") {
    ...
    if (matches.length > 1) {
      const kb = new InlineKeyboard();
      // сохраним кандидатов в сессии по имени → выбор через текст не нужен, дадим кнопки по раундам только если 1
