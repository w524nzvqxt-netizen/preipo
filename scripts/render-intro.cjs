...
const ROOT=path.resolve('.video-tmp/intro-20260907');const audioDir=path.resolve('public/uploads/v2/intro-20260907');fs.mkdirSync(audioDir,{recursive:true});
const fps=30;
...
(async()=>{
 ...
 for(const s of script.scenes){
  ...
  if(!fs.existsSync(out))execFileSync(FF,['-y','-i',raw,'-af','highpass=f=65,loudnorm=I=-16:TP=-1.5:LRA=7,adelay=300,apad=pad_dur=0.4','-ar','48000','-ac','2','-codec:a','libmp3lame','-b:a','192k',out],{stdio:'ignore'});
  ...
  const frames=Math.ceil((duration+.7)*fps);scenes.push({...s,audio:'uploads/v2/intro-20260907/'+s.id+'.mp3',frames,cues});time+=frames/fps;
 }
 const inputProps={scenes};fs.writeFileSync(path.join(ROOT,'render-props.json'),JSON.stringify(inputProps,null,2));fs.writeFileSync('public/uploads/preipo-intro-20260907.vtt',vtt);
 ...
 execFileSync(FF,['-y','-i',path.join(ROOT,'master.mp4'),'-c','copy','-movflags','+faststart','public/uploads/preipo-intro-20260907.mp4'],{stdio:'ignore'});
 await renderStill({composition,serveUrl,inputProps,output:'public/uploads/preipo-intro-20260907-poster.jpg',imageFormat:'jpeg',frame:90,browserExecutable});
 console.log('Ready: public/uploads/preipo-intro-20260907.mp4',fs.statSync('public/uploads/preipo-intro-20260907.mp4').size);
