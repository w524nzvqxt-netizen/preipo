// Главная страница — публичная витрина pre-IPO проектов (bento-раскладка)
...
// ISR: страница кэшируется и перегенерируется не чаще раза в 5 минут.
...
export default async function HomePage() {
  ...
  // Bento-витрина: спотлайт лучшей закрытой сделки + тайлы
  const spotlight =
  ...
  return (
    <div className="relative bg-bg">
      ...
      <main id="main" className="mx-auto w-full max-w-7xl px-6 pb-28">
        {/* Аналитика-баннер (наезжает на hero) */}
        <Link
          href="/exits"
        ...
        {/* 01 — Как это работает: bento (видео + что внутри) */}
        <section id="about" className="mt-24 scroll-mt-24">
          <SectionHead n="01" kicker="Инструкция для инвестора" title="Как это работает" />
          <div className="mt-8 grid grid-cols-1 gap-4 lg:grid-cols-12">
            ...
            <div className="flex flex-col justify-between rounded-card border border-border bg-surface p-6 lg:col-span-4">
              <div>
                <p className="kicker kicker-gold">Pre-IPO за 3 шага</p>
                <div className="mt-5 flex flex-col gap-4">
                  {[
                    { n: "01", t: "Оценка входа", d: "Цена, объём, оценка компании до IPO." },
                    { n: "02", t: "Структура и риски", d: "Условия сделки, ликвидность, документы." },
                    { n: "03", t: "Сценарий выхода", d: "IPO, вторичная продажа или M&A." },
                  ].map((s) => (
                    <div key={s.n} className="flex gap-3">
                      <span className="nums text-lg font-extrabold text-brand">{s.n}</span>
                      <div>
                        <h3 className="font-semibold text-text-primary">{s.t}</h3>
              ...
              <p className="mt-6 text-lg font-bold leading-tight text-text-primary">
                Лучшие компании мира — <span className="text-brand">до их выхода на биржу</span>.
              </p>
            </div>
          </div>
        </section>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            {FEATURES.map((f, i) => (
              <Reveal key={f.title} delay={i * 0.08}>
                <FeatureTile {...f} index={i} />
              </Reveal>
            ))}
          </div>
        </section>

        {/* Процесс — bento сетка тайлов */}
        <section id="process" className="mt-24 scroll-mt-24">
          <SectionHead n="02" kicker="Процесс" title="Как проходит сделка" />
          <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {PROCESS.map((s, i) => (
              <div key={s.title} className="card-premium rounded-card border border-border bg-surface p-6 transition-colors hover:border-brand/40">
                <span className="nums text-2xl font-extrabold text-brand">{String(i + 1).padStart(2, "0")}</span>
        ...
        <section id="deals" className="mt-24 scroll-mt-24">
          ...
          <div className="mt-10 grid grid-flow-row-dense grid-cols-2 gap-4 [grid-auto-rows:minmax(150px,auto)] lg:grid-cols-4">
            {spotlight && (
              <Link href={`/project/${spotlight.id}`} className="card-premium group relative col-span-2 row-span-2 flex flex-col justify-between overflow-hidden rounded-card border border-border bg-surface p-6 hover:-translate-y-1 hover:border-brand/50 hover:shadow-[var(--shadow-card-hover)] motion-reduce:hover:translate-y-0">
                <div className="pointer-events-none absolute right-[-20%] top-[-30%] h-64 w-64 rounded-full bg-brand/10 blur-[100px]" />
                <div className="relative flex items-center justify-between gap-3">
                  <p className="kicker kicker-gold">Закрытая сделка{spotlight.sector ? ` · ${spotlight.sector}` : ""}</p>
                  <span className="kicker shrink-0 rounded-pill border border-border bg-surface-alt px-2.5 py-1 text-text-muted">Закрыто</span>
                </div>
                <div className="relative">
                  <h3 className="text-display text-3xl font-bold sm:text-4xl">{spotlight.name}</h3>
                  <p className="mt-2 text-text-secondary">{[spotlight.sector, spotlight.stage].filter(Boolean).join(" · ") || "Late-stage private"}</p>
                </div>
                <div className="relative">
                  <div className="flex flex-wrap items-end justify-between gap-4">
                    <div>
                      <p className="kicker text-text-muted">Оценка входа</p>
                      <p className="nums text-3xl font-extrabold text-text-primary sm:text-4xl">{formatMoney(spotlight.valuation, spotlight.currency)}</p>
                    </div>
                  ...
                  {news.map((n) => (
                    <Link key={n.id} href="/news" className="group flex flex-col gap-1.5 py-3.5">
                      <div className="flex items-center gap-2">
                        {n.isHot && <span className="kicker rounded-full border border-warning/40 bg-warning/10 px-2 py-0.5 text-warning">Горячее</span>}
            ...
            <Link href="/base" className="card-premium group col-span-2 flex flex-col justify-between overflow-hidden rounded-card border border-border bg-surface p-6 hover:-translate-y-1 hover:border-brand/50 hover:shadow-[var(--shadow-card-hover)] motion-reduce:hover:translate-y-0">
              <div>
                ...
                <p className="mt-1.5 text-sm text-text-secondary">Оценки, раунды и разборы десятков pre-IPO компаний по секторам.</p>
              </div>
              <span className="mt-4 inline-flex items-center gap-1.5 text-sm font-medium text-brand">Открыть базу<span className="transition-transform group-hover:translate-x-1 motion-reduce:transition-none">→</span></span>
            </Link>
            ...
            <div className="relative flex flex-wrap items-center justify-between gap-6">
              <div className="max-w-2xl">
                ...
                <p className="mt-3 text-text-secondary">
                  Личный кабинет партнёра: клиенты и сделки, комиссии и выплаты, доходность
                  портфеля и отчёты — на сайте и в Telegram-боте.
                </p>
              ...
              <Link href="/agent" className="btn-brand shrink-0 rounded-control px-7 py-3.5 font-semibold">Войти в кабинет →</Link>
            </div>
          </div>
        </section>

        {/* Сравнение инструментов — bento tile с таблицей */}
        <section id="compare" className="mt-24 scroll-mt-24">
          <SectionHead n="04" kicker="Контекст" title="Место в портфеле" />
              ...
              <table className="w-full min-w-[680px] border-collapse text-sm">
                ...
                <tbody>
                  {[
                    ...
                    <tr key={row.name} className={`border-t border-border ${row.hl ? "bg-brand-subtle" : ""}`}>
        ...
        {/* Риски — bento сетка */}
        <section id="risks" className="mt-24 scroll-mt-24">
          <SectionHead n="05" kicker="Важно понимать" title="Риски Pre-IPO" />
          <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {RISKS.map((r) => (
          ...
          <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
            <div className="rounded-card border border-positive/25 bg-positive/[0.06] p-6 sm:p-8">
              ...
              <ul className="mt-4 space-y-3 text-sm leading-relaxed text-text-secondary">
                {["Квалифицированным инвесторам", "Тем, кто готов к горизонту 2–5 лет", "Тем, кто понимает риск private equity", "Кто хочет диверсификацию вне публичного рынка"].map((x) => (
                  <li key={x} className="flex gap-2.5"><span className="mt-0.5 text-positive">✓</span><span>{x}</span></li>
                ))}
              </ul>
            </div>
            <div className="rounded-card border border-warning/25 bg-warning/[0.05] p-6 sm:p-8">
              ...
              <ul className="mt-4 space-y-3 text-sm leading-relaxed text-text-secondary">
                {["Если нужны гарантии доходности", "Если деньги могут понадобиться через 3–6 месяцев", "Если вы не готовы к полной потере капитала", "Если вы хотите «быстрые иксы»"].map((x) => (
                  <li key={x} className="flex gap-2.5"><span className="mt-0.5 text-warning">—</span><span>{x}</span></li>
                ))}
              </ul>
            </div>
          </div>
        ...
        {/* Подбор сделок — bento tile с квизом */}
        <section id="quiz" className="mt-24 scroll-mt-24">
          <Reveal>
            <div className="hairline overflow-hidden rounded-card">
              <div className="grid sm:grid-cols-2">
                <div className="p-8 sm:p-10">
                  <p className="kicker kicker-gold">Подбор сделок</p>
                  <h2 className="text-display mt-2 text-2xl font-bold sm:text-4xl">Подберём сделки под ваш профиль</h2>
                ...
                <div className="island p-8 sm:p-10"><InvestorQuiz /></div>
              </div>
            </div>
          </Reveal>
        </section>
        ...
        <footer className="mt-20 border-t border-border pt-8 text-sm">
          ...
          <div className="mt-6 flex flex-wrap items-center gap-x-4 gap-y-2 text-text-muted">
            ...
            <Link href="/privacy" className="text-text-muted underline hover:text-text-secondary">Политика обработки персональных данных</Link>
      ...
      <div className="glass fixed inset-x-0 bottom-0 z-50 px-3 pt-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))] sm:hidden">
        <div className="flex gap-2">
          <a href="#quiz" className="btn-brand flex-1 rounded-control py-3 text-center font-semibold">Получить список сделок</a>
          {contacts.telegram && (
            <a href={contacts.telegram} target="_blank" rel="noopener noreferrer" className="hairline rounded-control px-5 py-3 text-center font-semibold text-text-primary">Telegram</a>
          )}
        </div>
      </div>
    </div>
...
function SectionHead({ n, kicker, title, gold }: { n: string; kicker: string; title: string; gold?: boolean }) {
  return (
    <div className="flex items-end gap-4">
      <span aria-hidden="true" className="nums text-[clamp(32px,5vw,72px)] font-extrabold leading-none text-border-strong">{n}</span>
      <div className="pb-1">
...
function Stat({ k, v, pos, gold, big }: { k: string; v: string; pos?: boolean; gold?: boolean; big?: boolean }) {
  return (
    <div>
      <p className="kicker text-text-muted">{k}</p>
      <p className={`nums font-bold ${big ? "text-2xl" : "text-base"} ${pos ? "text-positive" : gold ? "text-brand" : "text-text-primary"}`}>{v}</p>
    </div>
  );
}
...
function BentoClosed({ p }: { p: Project }) {
  ...
  return (
    <Link href={`/project/${p.id}`} className="card-premium group flex flex-col justify-between overflow-hidden rounded-card border border-border bg-surface p-5 hover:-translate-y-1 hover:border-brand/50 hover:shadow-[var(--shadow-card-hover)] motion-reduce:hover:translate-y-0">
...
const FEATURES: { title: string; text: string; accent: "positive" | "warning" | "accent" }[] = [
  { title: "Высокая потенциальная доходность", text: "Вход на стадии роста — по оценке частного раунда. Потенциал кратного роста доли к моменту выхода на биржу или продажи компании.", accent: "positive" },
  { title: "Риски", text: "Честно о главном: инвестиции в частные компании неликвидны, IPO и заявленная доходность не гарантированы, возможна полная потеря капитала.", accent: "warning" },
  { title: "Уникальные предложения", text: "Закрытые сделки и проекты, недоступные на открытом рынке. Отбираем вручную и сопровождаем вход в сделку.", accent: "accent" },
];

function FeatureTile({ title, text, accent, index }: { title: string; text: string; accent: "positive" | "warning" | "accent"; index: number }) {
  const borderAccent = accent === "positive" ? "border-l-positive" : accent === "warning" ? "border-l-warning" : "border-l-accent";
  return (
    <div className={`card-premium group h-full overflow-hidden rounded-card border border-border border-l-4 bg-surface p-6 hover:-translate-y-1 hover:shadow-[var(--shadow-card-hover)] motion-reduce:hover:translate-y-0 ${borderAccent}`}>
      <p className="kicker mb-3 text-text-muted">{String(index + 1).padStart(2, "0")}</p>
